...
int main()
{
	CreateCommand("Jump", A);
	CreateCommand("Punch", B);
	CreateCommand("Exit", Start);
	CreateCommand("Right", Shldr_RIGHT);
	CreateCommand("Left", Shldr_LEFT);

	CreateNewInputProfile("TestProfile");
	SetInputProfileBinding("Jump", "TestProfile", Shldr_LEFT);
	SetInputProfileBinding("Punch", "TestProfile", Shldr_RIGHT);

	CreateCommand("Delete", Y);
	CreateTriggerCommand("Shoot", 1, 0.1f);

	SetInputProfileBinding("Shoot", "TestProfile", A);

	InitGamepads();

	while (!GetCommandDown(0, "Exit"))
	{
		if (GetControllerKeyDown(0, Back))
		{
			if (g_Players[0].m_profile == &g_DefaultProfile)
			{
				g_Players[0].m_profile = &g_Profiles["TestProfile"];
			}
			else
			{
				g_Players[0].m_profile = &g_DefaultProfile;
			}
		}
		
		if (GetControllerKeyReleased(0, Shldr_RIGHT))
		{
			SetControllerRumbleTime(0, 0.f, 1.f, 0.2f);
		}
		if (GetControllerKeyReleased(0, Shldr_LEFT))
		{
			SetControllerRumbleTime(0, 1.f, 0.f, 0.2f);
		}
		if (GetCommandDown(0, "Delete"))
		{
			DeleteCommand("Punch");
		}
		cout <<
			(CheckGamepadConnected(0)
				+ CheckGamepadConnected(1)
				+ CheckGamepadConnected(2)
				+ CheckGamepadConnected(3))
			<< " Connected" << std::endl <<
			g_Players[0].m_profile->m_Name << 
			endl <<
			"Jump " << GetCommandDown(0, "Jump") << endl <<
			"Shoot " << GetCommandDown(0, "Shoot") << endl <<
			"Punch " << GetCommandDown(0, "Punch") << endl <<
			"Right " << GetCommandDown(0, "Right") << endl <<
			"Left " << GetCommandDown(0, "Left") << endl;
		Sleep(10);
		system("CLS");
	}
	CreateNewCopiedInputProfile("TestProfile", "CopiedProfile");
	SetInputProfileDeadzones("CopiedProfile", 0.3f, 0.2f, 0.1f, 0.8f);
	SaveInputProfilesToDisk("testProfiles.txt");

	system("PAUSE");


	/*c = new XInput_Gamepad[4];
	c[0].InitGamepad(0);
	c[1].InitGamepad(1);
	c[2].InitGamepad(2);
	c[3].InitGamepad(3);
	c[1].setDeadZones(0.5, 0.5, 0.5, 0.5);

	while (!c[1].GetButtonPressed(Start))
	{
		std::cout <<
			(c[0].connected() + c[1].connected() + c[2].connected() + c[3].connected()) << " Connected" << std::endl <<
			"A " << c[1].GetButtonPressed(A) << std::endl <<
			"B " << c[1].GetButtonPressed(B) << std::endl <<
			"X " << c[1].GetButtonPressed(X) << std::endl <<
			"Y " << c[1].GetButtonPressed(Y) << std::endl <<
			std::endl <<
			"DP_U " << c[1].GetButtonPressed(DPad_UP) << std::endl <<
			"DP_D " << c[1].GetButtonPressed(DPad_DOWN) << std::endl <<
			"DP_L " << c[1].GetButtonPressed(DPad_LEFT) << std::endl <<
			"DP_R " << c[1].GetButtonPressed(DPad_RIGHT) << std::endl <<
			std::endl <<
			"Shldr_L " << c[1].GetButtonPressed(Shldr_LEFT) << std::endl <<
			"Shldr_R " << c[1].GetButtonPressed(Shldr_RIGHT) << std::endl <<
			std::endl <<
			"Start " << c[1].GetButtonPressed(Start) << std::endl <<
			"Back " << c[1].GetButtonPressed(Back) << std::endl <<
			std::endl <<
			"Thmb_L " << c[1].GetButtonPressed(ThumbStick_LEFT) << std::endl <<
			"Thmb_R " << c[1].GetButtonPressed(ThumbStick_RIGHT) << std::endl <<
			std::endl <<
			"L_Stick_X " << c[1].LeftStick_X() << std::endl <<
			"L_Stick_Y " << c[1].LeftStick_Y() << std::endl <<
			std::endl <<
			"R_Stick_X " << c[1].RightStick_X() << std::endl <<
			"R_Stick_Y " << c[1].RightStick_Y() << std::endl <<
			std::endl <<
			"L_Trig " << c[1].LeftTrigger() << std::endl <<
			"R_Trig " << c[1].RightTrigger() << std::endl;
		c[1].setRumble(c[1].LeftTrigger(), c[1].RightTrigger());
		Sleep(10);
		system("CLS");
	}*/
	return 0;
